...
DECLARE @TableHtml NVARCHAR(MAX) = (
 ...
 HAVING  --is running
         ...
         DATEDIFF(MINUTE, MAX(sja.[start_execution_date]), GETDATE()) > 30 AND
          DATEDIFF(MINUTE, MAX(sja.[start_execution_date]), GETDATE()) > 1.5 *MAX(AVERAGES.[avg_duration_in_mins])
 FOR     --wrap in HTML table row element
         XML RAW('tr'), ELEMENTS
)

--sanity check: did we get any jobs that have been running for longer than 30 minutes and 1.5 time the average ? If not, leave
...
EXEC msdb.dbo.sp_send_dbmail @recipients = 'email@plansis.nl', @subject = @EmailSubject, @body = @EmailBody, @body_format = 'HTML'
